...
from chatgpt.nn import BLOOMRM, GPTRM, T5RM
...
def train(args):
    ...
    tokenizer = GPT2TokenizerFast.from_pretrained(args.pretrain)
    # tokenizer = T5TokenizerFast.from_pretrained(args.pretrain)
    tokenizer.pad_token = tokenizer.eos_token
    with strategy.model_init_context():
        # model = BLOOMRM(pretrained=args.pretrain).cuda()
        model = GPTRM(pretrained=args.pretrain).cuda()
        # model = T5RM(pretrained=args.pretrain).cuda()
    max_len = 512
    ...
    train_data = data["train"].select(range(60000))
    eval_data = data['test'].select(range(5000))
