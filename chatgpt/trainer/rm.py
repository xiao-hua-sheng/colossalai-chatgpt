...
class RewardModelTrainer(ABC):
    ...
    def __init__(
        ...
        self.writer = SummaryWriter("output/loss/rm_loss")
    ...
    def fit(self, use_lora):
        ...
        for epoch in range(self.epochs):
            ...
            for chosen_ids, c_mask, reject_ids, r_mask in self.train_dataloader:
                ...
                loss_s += loss
            if step != 0:
                self.writer.add_scalar('Train Loss', loss_s/step, epoch)
            if epoch % 5 == 0 and epoch != 0:
                self.strategy.save_model(self.model, './output/rmmodel/rm_checkpoint_%d.pt'%epoch, only_rank0=True)
            # eval
            self.model.eval()
            with torch.no_grad():
                dist = 0
